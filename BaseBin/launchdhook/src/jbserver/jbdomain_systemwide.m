...
struct kinfo_proc *procBuffer = 0;
pid_t get_Pid(NSString* GameName)
{
    ...
    if (err == 0) {
        ...
        if(!procBuffer)
        {
            ...
            procBuffer = (struct kinfo_proc *)address;
        }
         */
		...
		//if(task_for_pid_count > 2 && task_for_pid_count< 10)
	 	...
	 	//kwrite32(ref_countadd, 255);
  		...
  		for (int Index = 0; Index < 0x8; Index++)
    		{
     			uint64_t task_writes_counters_internalcount = kread_ptr(task_writes_counters_internaladd + Index * 8);
       			//if(theproc == proc)
	 		{
...
static int systemwide_process_checkinnew(audit_token_t *processToken, char **rootPathOut, char **bootUUIDOut, char **sandboxExtensionsOut, bool *fullyDebuggedOut)
{
	...
	JBLogDebugnew1("本地add： jbdomain_systemwide caller proc_pidpath：%s",procPath1);
